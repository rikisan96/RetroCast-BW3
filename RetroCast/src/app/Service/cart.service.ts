...
export class CartService {
      ...
      return new Observable<any>((observer) => {
        this.getCartGames(this.user.id).subscribe((cartItems) => {
          ...
          this.getPurchasedGames(this.user.id).subscribe((purchasedGames) => {
            const alreadyPurchased = purchasedGames.some((bg) =>
              bg.cartItems.some((item) => item.game.id === game.id)
            );
            if (alreadyPurchased) {
            ...
            this.http
              .post<iGameList>(this.cartUrl, requestBody)
              .pipe(
                tap(() => {
  ...
  getCartGamesLength(): Observable<number> {
    if (this.user) {
      return this.http
        .get<ICartItem[]>(`${this.cartUrl}?userId=${this.user.id}`)
        .pipe(
  ...
  getPurchasedGames(userId: number): Observable<Iboughtgames[]> {
    return this.http.get<Iboughtgames[]>(
      `${this.boughtGamesUrl}?userId=${userId}`
    );
  }
  ...
  purchaseGames(): Observable<void> {
    ...
    return new Observable<void>((observer) => {
      this.getCartGames(this.user.id).subscribe((cartItems) => {
        ...
        this.http
          .get<Iboughtgames[]>(`${this.boughtGamesUrl}?userId=${this.user.id}`)
          .subscribe((boughtGames) => {
            let userBoughtGames = boughtGames.find(
              (bg) => bg.userId === this.user.id
            );
            ...
            cartItems.forEach((cartItem) => {
              const existingBoughtItem = userBoughtGames!.cartItems.find(
                (boughtItem) => boughtItem.game.id === cartItem.game.id
              );
              if (!existingBoughtItem) {
                userBoughtGames!.cartItems.push(cartItem);
              } else {
                console.error(
                  `Game "${cartItem.game.Title}" is already purchased.`
                );
              }
            });
            ...
            if (userBoughtGames!.id === Date.now()) {
              this.http
                .post(this.boughtGamesUrl, userBoughtGames)
                .subscribe(() => {
              ...
              this.http
                .put(
                  `${this.boughtGamesUrl}/${userBoughtGames.id}`,
                  userBoughtGames
                )
                .subscribe(() => {
                  this.clearCart(this.user.id).subscribe(() => {
